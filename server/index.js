...
const { prime, tartget_port, proxy_server_port } = process.env
...
const server_base = createDiffieHellman(prime, "hex", 2);
...
app.use(async (req, res, next) => {
    if (!keysMap[req.ip])
        ...
        proxy.web(req, res, { target: `http://base-server:${tartget_port}`, selfHandleResponse: true })
    }
});

http.createServer(app).listen(proxy_server_port, '0.0.0.0', () => {
    console.log(`Proxy server linsten on ${proxy_server_port}`);
});