...
const { prime, proxy_server_port, proxy_client_port } = process.env
const proxy = httpProxy.createProxyServer({})
const app = express();
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

const client_base = createDiffieHellman(prime, "hex", 2);
...
const keyGen = async () => {
    // get server key
    share_key = await _fetch(`http://proxy-sender:${proxy_server_port}/key_exchange`, {
        method: 'POST',
...
const reSend = async req => {
    ...
    // resend the request
    body = await _fetch(`http://proxy-sender:${proxy_server_port}${req.url}`, {
        method: req.method,
        headers: req.headers,
...
app.use(async (req, res) => {
    ...
    proxy.web(req, res, { target: `http://proxy-sender:${proxy_server_port}`, selfHandleResponse: true })
});

http.createServer(app).listen(proxy_client_port, '0.0.0.0', () => {
    console.log(`Proxy client linsten on ${proxy_client_port}`);
});